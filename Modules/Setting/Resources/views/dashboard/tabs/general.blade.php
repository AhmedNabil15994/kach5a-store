...
{{--    <h3 class="page-title">{{ __('setting::dashboard.settings.form.tabs.general') }}</h3>--}}
    <div class="col-md-10">
        ...
        @php
            $def_currency =  \Modules\Area\Entities\CurrencyCode::find($default_currency);
        @endphp
        @foreach ($currencies as $lastCurrency)
            @if (in_array($lastCurrency->id, $supported_currencies ?? []) && $lastCurrency->id != $default_currency)
                <div class="form-group">
                    <label class="col-md-2">
                        {{$def_currency->translate('name',locale())}} {{ __("setting::dashboard.to") }} {{$lastCurrency->translate('name',locale())}}
                    </label>
                    <div class="col-md-9">
                        <input type="text" class="form-control" name="{{$def_currency->code}}_to_{{$lastCurrency->code}}" value="{{Setting::get($def_currency->code.'_to_'.$lastCurrency->code)}}"
                               autocomplete="off"/>
                    </div>
                </div>
            @endif
