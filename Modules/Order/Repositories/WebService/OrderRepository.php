...
class OrderRepository
{
    ...
    public function create($request, $userToken = null)
    {
        ...
        try {
            ...
            $now = Carbon::now();
            $shipping_type = isset($request->shipping['type']) ?  $request->shipping['type'] : 'direct';
            $date = $now->format('Y-m-d');
            $time_from = $now->format('h:i A');
            $time_to = $now->format('h:i A');
            if (isset($request->shipping['type'])) {
                if ($request->shipping['type'] == 'schedule') {
                    $date =  $request->shipping['date'];
                    $time_from = $request->shipping['time_from'];
                    $time_to = $request->shipping['time_to'];
                }
            }
